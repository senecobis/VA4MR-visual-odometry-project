...
subplot(2,2,1)
imshow(img,[]); hold on
plot(S.p(1,1:num_keyp-cont), S.p(2,1:num_keyp-cont),'ob');
plot(S.p(1,num_keyp-cont:num_keyp), S.p(2,num_keyp-cont:num_keyp),'or');
...
subplot(2,2,[3 4])
plot(newpos(:,1), newpos(:,3), 'LineWidth', 3); hold on
plot(S.X(1,:),S.X(3,:),'o');
ylim([newpos(end,3)-20, newpos(end,3)+50]);
xlim([newpos(end,1)-20, newpos(end,1)+50]);
axis equal
